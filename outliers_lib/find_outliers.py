...
def find_outliers_quantile(data, feature, left=0.01, right=0.99):
    ...
    Args:
        data (pandas.DataFrame): набор данных
        feature (str): имя признака, на основе которого происходит поиск выбросов
        left (float, optional): уровень значимости, по которому данные принимаеются как выбросы; по умолчанию 0.01.
        right (float, optional): уровень значимости, по которому данные принимаются как нормальные
